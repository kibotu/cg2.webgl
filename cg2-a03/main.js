...
define(["jquery", "gl-matrix", "utils/webgl-debug", "scene/scene", "animations/animation", "scene/scene_explorer", "controller/html_controller" ],
    (function ($, glmatrix, WebGLDebugUtils, Scene, Animation, SceneExplorer, HtmlController) {
        ...
        $(document).ready((function () {
            ...
            var lightAnimation = new Animation((function (t, deltaT) {
                ...
                mat4.rotate(scene.sunNodePlanet.transformation, angle, [0, 1, 0]);

                // rotate planet
                mat4.rotate(scene.planetNode.transformation, angle / 10, [0, 0, 1]);

                // rotate clouds
                scene.cloudShift[0] += angle / 40;

                // alternate cloud intensity
                scene.cloudsIntensity = Math.sin(t * 0.0007) + 1.5;

                // month iteration
                scene.dt += deltaT;
                if(scene.dt >= 90) {
                    scene.month = (scene.month + 1) % 12;
                    scene.dt = 0;
                }
