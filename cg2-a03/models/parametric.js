    ...
    (function (Util, vbo) {
        ...
        var ParametricSurface = function (gl, definition, config) {
            ...
            definition = definition || {
                ...
                "binormal":function (u, v) {
                    return [0, 1, 0];
                },
                "tangent":function (u, v) {
                    return [1, 0, 0];
                },
                "texCoords":function (u, v) {
                    return [u, v];
                }
            };
            ...
            var vbinormals = new Array(vposition.length);
            var vtangents = new Array(vposition.length);
            ...
            for (i = 0; i < vposition.length; i += 3) {
                vec3.normalize([vnormal[i + 0],vnormal[i + 1],vnormal[i + 2]],w);
                vec3.normalize([0.0,30.0,1.0],b);
                vec3.normalize(vec3.cross(w,b,tmp),s);
                vec3.normalize(vec3.cross(s,w,tmp),t);
            ...
            if (definition.binormal)
                this.normalBuffer = new vbo.Attribute(gl, {"numComponents":3, "dataType":gl.FLOAT, "data":vbinormals});
            if (definition.tangent)
                this.normalBuffer = new vbo.Attribute(gl, {"numComponents":3, "dataType":gl.FLOAT, "data":vtangents});
            if (definition.texCoords)
